<template>
	<div>
		<n8n-menu-item v-for="item in items" :key="item.id" :index="item.id" @click="onClick(item)">
			<font-awesome-icon :icon="item.properties.icon" />
			<span slot="title" :class="{ 'item-title-root': root, 'item-title': !root }">{{
...
export default Vue.extend({
	...
	props: ['items', 'root', 'afterItemClick'],
	methods: {
